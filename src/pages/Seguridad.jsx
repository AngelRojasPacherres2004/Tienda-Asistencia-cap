...
export default function Seguridad({ user }) {
  ...
  return <>
    ...
    <div className="security-home-layout"><div>
      <div className="security-metrics"><Metric icon={UsersRound} tone="green" label="Tráfico hoy" value={todayTraffic?.cantidad ?? 0} footer={todayTraffic ? `Registrado · ${timeOf(todayTraffic.updated_at || todayTraffic.created_at)}` : "Registro pendiente"} /><Metric icon={ShieldAlert} tone="amber" label="Incidencias abiertas" value={openIncidents.length} footer={`${openIncidents.filter((r) => ["alta", "critica"].includes(r.gravedad)).length} de alta severidad`} /><Metric icon={Clock3} tone="red" label="Pendientes" value={pending} footer={pending ? "Acción requerida" : "Todo al día"} /></div>
