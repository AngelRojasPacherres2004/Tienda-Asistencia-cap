...
export default function TraficoSeguridad({ user }) {
  ...
  const [form, setForm] = useState({ fecha: todayISO(), cantidad: "", observaciones: "" });
  ...
  const save = async (e) => { e.preventDefault(); try { await api("/trafico", { method: "POST", body: form }); setOpen(false); setForm({ fecha: todayISO(), cantidad: "", observaciones: "" }); await load(); } catch (error) { setNotice({ type: "error", text: error.message }); } };
  const exportCsv = () => downloadCsv("trafico-tienda.csv", ["Fecha,Visitantes,Observación", ...(rows || []).map((r) => [r.fecha, r.cantidad, quoted(r.observaciones)].join(","))]);
  if (!rows) return <Loading />;
  return <>
    <PageHeader eyebrow="Seguridad" title="Tráfico de clientes" subtitle="Registro diario e histórico de afluencia" action={<button className="button button--primary" onClick={() => setOpen(true)}><Plus size={16} /> Registrar tráfico</button>} />
