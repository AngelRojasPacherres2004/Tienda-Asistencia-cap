...
export default function ErroresAmonestaciones({ user }) {
  ...
  const [section, setSection] = useState("errores");
  ...
  const load = useCallback(() => Promise.all([api("/incidentes"), api("/amonestaciones"), api("/usuarios")])
    ...
    .catch((error) => setNotice({ type: "error", text: error.message })), [isGerente]);
  useEffect(() => { load(); }, [load]);
  const scopedPersonal = useMemo(() => personal.filter((person) => person.rol === (isGerente ? "jefe_zonal" : "administrador_tienda")), [personal, isGerente]);
  ...
  return <>
    <PageHeader eyebrow="Gestión zonal" title="Errores y amonestaciones" subtitle="Consulta los errores reportados y administra las amonestaciones de los responsables de tienda." />
    {notice && <Notice type={notice.type} onClose={() => setNotice(null)}>{notice.text}</Notice>}
    <nav className="schedule-tabs"><button className={section === "errores" ? "active" : ""} onClick={() => setSection("errores")}><ShieldAlert size={16} />Errores</button><button className={section === "amonestaciones" ? "active" : ""} onClick={() => setSection("amonestaciones")}><AlertTriangle size={16} />Amonestaciones</button></nav>
    {section === "errores" ? (!errores ? <Loading /> : errores.length ? <div className="incident-list">{errores.map((item) => <article className="incident-card" key={item.id}><span className="incident-card__icon"><ShieldAlert size={20} /></span><div><header><h3>{item.titulo}</h3><time>{formatDateTime(item.fecha_creacion)}</time></header><p>{item.descripcion}</p><small>Reportado por {item.reportado_por_nombre || "Seguridad"}</small></div></article>)}</div> : <EmptyState icon={ShieldAlert} title="Sin errores" text="No hay errores reportados." />) : <>
      <section className="panel warning-summary"><header className="panel__header"><div><small>Resumen</small><h2>Amonestaciones por usuario</h2></div></header>{!amonestaciones ? <Loading /> : <div className="table-panel"><div className="data-table data-table--warning-summary"><div className="data-table__head"><span>Trabajador</span><span>Usuario</span><span>Rol</span><span>Amonestaciones</span></div>{summary.map((item) => <div className="data-table__row" key={item.id}><strong>{item.nombres} {item.apellidos}</strong><span>@{item.usuario}</span><span>Administrador de tienda</span><strong>{item.cantidad}</strong></div>)}</div></div>}</section>
