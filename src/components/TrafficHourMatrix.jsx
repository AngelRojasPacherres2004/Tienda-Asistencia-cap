...
export default function TrafficHourMatrix({ user, tiendaId = "", scopeName = "", period }) {
  ...
  const selectedDates = useMemo(() => {
    ...
    const today = isoDate(new Date()); const daysInMonth = new Date(Number(year), Number(monthNumber), 0).getDate();
  ...
  const desde = dates.at(-1) || isoDate(new Date()); const hasta = dates[0] || desde;
  const load = useCallback(() => { setLoading(true); setError(""); const params = new URLSearchParams({ desde, hasta, ...(tiendaId ? { tienda_id: tiendaId } : {}) }); api(`/trafico/matriz?${params}`).then(setData).catch((e) => setError(e.message)).finally(() => setLoading(false)); }, [desde, hasta, tiendaId]);
  useEffect(() => { load(); }, [load]);
  ...
  const view = useMemo(() => {
    const values = new Map(); const stores = new Set(); for (const row of data?.registros || []) { const key = `${row.fecha}|${row.rango_hora}`; values.set(key, (values.get(key) || 0) + Number(row.cantidad || 0)); stores.add(row.tienda_id); }
  ...
  const colorRanges = useMemo(() => { if (!view.maximum) return []; const step = Math.ceil(view.maximum / 5); return Array.from({ length: 5 }, (_, index) => { const start = index * step + 1; const end = Math.min((index + 1) * step, view.maximum); return { level: index + 1, label: start === end ? `${start}` : `${start}–${end}` }; }); }, [view.maximum]);
  ...
  return <section className={`traffic-matrix-card ${expanded ? "is-expanded" : ""}`}>
    ...
    <div className="traffic-matrix-kpis"><span><UsersRound size={18} /><small>Visitas registradas</small><strong>{view.total.toLocaleString("es-PE")}</strong></span><span><Activity size={18} /><small>Hora de mayor tráfico</small><strong>{view.total ? rangeLabel(view.peak) : "Sin datos"}</strong></span><span><CalendarDays size={18} /><small>Promedio diario</small><strong>{Math.round(view.total / Math.max(dates.length, 1)).toLocaleString("es-PE")}</strong></span>{user?.rol !== "jefe_tienda" && <span><Store size={18} /><small>Tiendas con registros</small><strong>{view.stores}</strong></span>}</div>
